  ...
  var callbackFunctions = {
    startSuite: function (start) {
      ...
      $.each(
        [
          "browseraudit.com",
          "browseraudit.com/sop/path",
          "test.browseraudit.com",
          "browseraudit.org",
          "test.browseraudit.org",
        ],
        function (i, v) {
      ...
      testSandbox.waitForImages(function () {
        ...
        testSandbox.waitForImages(function () {
          ...
          $("<iframe>", { src: "/static/reset_local_storage.html" })
            .appendTo(testSandbox)
            .load(function () {
              $("<iframe>", { src: "https://test.browseraudit.com/static/reset_local_storage.html" })
                .appendTo(testSandbox)
                .load(function () {
                  ...
                  browserAuditUI.notificationBar.setMessage(
                    "play",
                    "The test suite is currently running. Click here to pause execution.",
                    function () {
    ...
    pauseSuite: function () {
      browserAuditUI.notificationBar.setMessage(
        "pause",
        "The test suite is currently paused. Click here to resume execution.",
        function () {
    ...
    endSuite: function (result) {
      ...
      if (browserAuditSettings.sendresults) {
        var sendResults = function () {
          browserAuditUI.notificationBar.setMessage(
            "cloud-upload",
            "Sending test results to BrowserAudit&hellip;",
            null,
          );
          ...
          $.ajax({
            ...
            success: function (data, status, xhr) {
              ...
              browserAuditUI.notificationBar.setMessage(
                "off",
                'The test suite has finished executing. You can refer to these test results again at any time using <a href="/results/' +
                  res[0] +
                  "/" +
                  res[1] +
            ...
            error: function (xhr, status, err) {
              ...
              if (xhr.status == 400) {
                browserAuditUI.notificationBar.setMessage(
                  "off",
                  "The test suite has finished executing, but there was a problem with the test results sent to BrowserAudit. If you would like to record your test results, click here to run the test suite again.",
                  function () {
  ...
  if (browserAuditSettings.displaymode === "full") {
    ...
    callbackFunctions.endCategory = function (result) {
      ...
      if (result.critical > 0) {
        ...
        thisCategory.setOutcome("critical", result.critical);
      }
      if (result.warning > 0) {
        allTestsPassed = false;
        thisCategory.setOutcome("warning", result.warning);
      }
      if (result.skip > 0) {
        allTestsPassed = false;
        thisCategory.setOutcome("skip", result.skip);
      }
    ...
    callbackFunctions.startTest = function () {
      thisTest = browserAuditUI.testReport(
        this.id,
        this.title,
        this.behaviour,
        this.timeout,
        this.testFunction,
        this.testFunction.reportData,
      );
